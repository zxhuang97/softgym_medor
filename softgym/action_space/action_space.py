...
class Picker(ActionToolBase):
    def __init__(self, num_picker=1, picker_radius=0.05, init_pos=(0., -0.1, 0.), picker_threshold=0.00625,
                 particle_radius=0.05,
                 picker_low=(-0.4, 0., -0.4), picker_high=(0.4, 0.5, 0.4), init_particle_pos=None, spring_coef=1.2,
                 pc_mass_ratio=0, pc_force_reset=False,
                 **kwargs):
        """
        ...
        self.pc_mass_ratio = pc_mass_ratio
        self.pc_force_reset = pc_force_reset
    ...
    def step(self, action):
        ...
        if pc_action is not None:
            for i in range(self.num_picker):
                if self.picked_particles[i] is not None:
                    pc_action[self.picked_particles[i], 3] = 0
            modify_particle_positions(pc_action, pc_mass_ratio=self.pc_mass_ratio,
                                      pc_force_reset=self.pc_force_reset)
...
class ClothPicker(ActionToolBase):
    def __init__(self, num_picker=1, picker_radius=0.05, init_pos=(0., -0.1, 0.), picker_threshold=0.00625,
                 particle_radius=0.05,
                 picker_low=(-0.4, 0., -0.4), picker_high=(0.4, 0.5, 0.4), init_particle_pos=None, mode='nearest',
        ...
        self.picker_threshold = picker_threshold
        self.num_picker = num_picker
        self.picked_particles = [None] * self.num_picker  # each picker has a list of picked particles
        self.picker_low, self.picker_high = np.array(list(picker_low)), np.array(list(picker_high))
        self.init_pos = init_pos
        self.particle_radius = particle_radius
        self.init_particle_pos = init_particle_pos
        assert mode in ['nearest', 'cylinder']
        self.mode = mode  # nearest, cylinder
        self.spring_coef = spring_coef  # Prevent picker to drag two particles too far away
        self.is_hide = False
        self.prev_shape_states = None
        space_low = np.array([-0.1, -0.1, -0.1, 0] * self.num_picker) * 0.1  # [dx, dy, dz, [0, 1]]
    ...
    def _apply_picker_boundary(self, picker_pos):
        clipped_picker_pos = picker_pos.copy()
        for i in range(3):
            clipped_picker_pos[i] = np.clip(picker_pos[i], self.picker_low[i] + self.picker_radius,
    ...
    def reset(self, center):
        ...
        self.picked_particles = [[]] * self.num_picker
        shape_state = np.array(pyflex.get_shape_states()).reshape(-1, 14)
        centered_picker_pos = self._get_centered_picker_pos(center)
        for (i, centered_picker_pos) in enumerate(centered_picker_pos):
            shape_state[i] = np.hstack([centered_picker_pos, centered_picker_pos, [1, 0, 0, 0], [1, 0, 0, 0]])
    ...
    def step(self, action):
        ...
        # store the picked particle position after the pciker movement
        self.intermediate_picked_particle_pos = []

        # If picker is not in picked mode, but there are picked particles, unpick them
        for i in range(self.num_picker):
            if not pick_flag[i] and len(self.picked_particles[i]) > 0:
        ...
        # Pick new particles and update the mass and the positions
        # If picker is in the picked mode, but the picked particles haven't been selected, search for picked particles
        # by nearest neighbor or cylinder query (within radius on the horizontal plane)
        for i in range(self.num_picker):
            new_picker_pos[i, :] = self._apply_picker_boundary(picker_pos[i, :] + action[i, :3])
            if pick_flag[i] and len(self.picked_particles[i]) == 0:  # if picker is in
                if self.mode == 'nearest':
                    dists = scipy.spatial.distance.cdist(picker_pos[i].reshape((-1, 3)),
                                                         particle_pos[:, :3].reshape((-1, 3)))
                    mask = dists.flatten() <= self.picker_threshold + self.picker_radius + self.particle_radius
                    if np.sum(mask) > 0:
                        pick_id = dists.argmin()
                        self.picked_particles[i].append(pick_id)
                elif self.mode == 'cylinder':
                    picker_pos_2d = picker_pos[i, [0, 2]].reshape(-1, 2)
                    particle_pos_2d = particle_pos[:, [0, 2]].reshape(-1, 2)
                    idx = np.arange(particle_pos.shape[0])
                    dists = scipy.spatial.distance.cdist(picker_pos_2d, particle_pos_2d)
                    mask = dists.flatten() <= self.picker_threshold + self.particle_radius
                    # TODO(zixuan): add height mask so that it won't grasp too many layers
                    picked_idx = idx[mask]
                    self.picked_particles[i] = picked_idx

            if len(self.picked_particles[i]) > 0:
                # TODO The position of the particle needs to be updated such that it is close to the picker particle
                new_particle_pos[self.picked_particles[i], :3] = particle_pos[self.picked_particles[i], :3] + \
                                                                 new_picker_pos[i:i + 1, :] - picker_pos[i:i + 1, :]
                new_particle_pos[self.picked_particles[i], 3] = 0  # Set the mass to infinity
                self.intermediate_picked_particle_pos.append(new_particle_pos[self.picked_particles[i], :3])

        # NOTE: doesn't seem to take effect
        # check for e.g., rope, the picker is not dragging the particles too far away that violates the actual physicals constraints.
        if self.init_particle_pos is not None:
            picked_particle_idices = []
            active_picker_indices = []
            ...
            for i in range(l):
                for j in range(i + 1, l):
                    ...
                    if now_distance >= init_distance * self.spring_coef:  # if dragged too long, make the action has no effect; revert it
                        new_picker_pos[active_picker_indices[i], :] = picker_pos[active_picker_indices[i], :].copy()
                        new_picker_pos[active_picker_indices[j], :] = picker_pos[active_picker_indices[j], :].copy()
                        new_particle_pos[picked_particle_idices[i], :3] = particle_pos[picked_particle_idices[i],
                                                                          :3].copy()
                        new_particle_pos[picked_particle_idices[j], :3] = particle_pos[picked_particle_idices[j],
                                                                          :3].copy()

        self._set_pos(new_picker_pos, new_particle_pos)


def modify_particle_positions(action, pc_mass_ratio=0, pc_force_reset=False):
    # Directly change the position of large amount of particles
    # action: N x 4, where N is the number of particles. First 3 dimensions are the movement, last dimension picked
    # status.
    # change the position
    ori_particle_pos = np.array(pyflex.get_positions()).reshape(-1, 4)
    new_particle_pos = ori_particle_pos.copy()
    particle_move = action[:, :3] * action[:, 3:4]
    new_particle_pos[:, :3] += particle_move
    move_flag = action[:, 3].astype(np.bool)
    new_particle_pos[move_flag, 3] *= pc_mass_ratio  # set to infinity
    pyflex.set_positions(new_particle_pos)

    pyflex.step()
    if action.shape[1] == 10:
        cur_vel = pyflex.get_velocities().reshape(-1, 3)
        velocities_tgt = action[:, 7:10] * action[:, 3:4] + cur_vel * (1 - action[:, 3:4])
        pyflex.set_velocities(velocities_tgt)
    if action.shape[1] == 7 or action.shape[1] == 10:
        cur_vel = pyflex.get_velocities().reshape(-1, 3)
        velocities_tgt = action[:, 4:7] * action[:, 3:4] + cur_vel * (1 - action[:, 3:4])
        pyflex.set_velocities(velocities_tgt)

    # change the mass back
    particle_pos = np.array(pyflex.get_positions()).reshape(-1, 4)
    if pc_mass_ratio != 0 and pc_force_reset:  # set the position by force
        particle_pos[move_flag] = new_particle_pos[move_flag]
    particle_pos[:, 3] = ori_particle_pos[:, 3]
